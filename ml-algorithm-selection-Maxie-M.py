...
#prep models 
models = []
models.append(('decTr', DecisionTreeClassifier(random_state = 0, max_depth = 10)))
models.append(('ranFor', RandomForestClassifier(random_state = 0, n_estimators=100, max_depth = 10)))
models.append(('logReg', LogisticRegression(random_state = 0)))
#evaluate each model (training dataset)
results = []
names = []
scoring = 'accuracy'
...
for name, model in models:
    ...
    cv_res_rw = model_selection.cross_val_score(model, X_train, y_train, cv = kfold, scoring = scoring)
    results.append(cv_res_rw)
    names.append(name)
    msg = "%s: %f (%f)" % (name, cv_res_rw.mean(), cv_res_rw.std())
    print(msg)


#visualizing the comparison (training dataset)
fig = plt.figure()
fig.suptitle('Algorithm Comparison (Training Dataset)')
...
#evaluate each model (testing dataset)
results = []
names = []
scoring = 'accuracy'
...
for name, model in models:
    kfold = model_selection.KFold(n_splits = 10, random_state = None)
    cv_res_rw = model_selection.cross_val_score(model, X_test, y_test, cv = kfold, scoring = scoring)
    results.append(cv_res_rw)
    names.append(name)
    msg = "%s: %f (%f)" % (name, cv_res_rw.mean(), cv_res_rw.std())
    print(msg)
...
#visualizing the comparision (testing dataset)
fig = plt.figure()
fig.suptitle('Algorithm Comparison (Testing Dataset)')
ax = fig.add_subplot(111)
plt.boxplot(results)
ax.set_xticklabels(names)
plt.show()
